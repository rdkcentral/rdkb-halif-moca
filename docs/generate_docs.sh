...
DOXYGEN_EXTRA_PARAMS="PREDEFINED='MOCA_VAR=0'"

# Check if the common document configuration is present, if not clone it
if [ -d "./build" ]; then
    make -C ./build PROJECT_NAME="RDK-B MOCA HAL" PROJECT_VERSION=${PROJECT_VERSION} DOXYGEN_EXTRA_PARAMS="${DOXYGEN_EXTRA_PARAMS}"
else
